...
- ciężkie pierwiastki (cięższe od żelaza) powstają w wyniku wychwytu neutronu (umierające małe gwiazdy (s) bądź wybuchy supernowych/zderzenia gwiazd neutronowych (r))
...
granice płyt to grzbiety i rowy oceaniczne oraz uskoki transformująće.
Gó©y mogą powstawać poprzez wsuwanie się jednej płyty pod drugą.
...
najważniejszymi materiałąmi skałotwó©czymi są glinokrzemiany i krzeminay
glinokrzemiany stanowią 60% masy skorupy
czworościany krzemianowo-tlenowe łąćzą się w polimery. gdy cześć krzemu zostanie zamieniona glinem tworzą się glinokrzemiany.
...
pole ziemskie możńa podzielić na 3 skłądowe przestrzenne (x,y,z). Można wyznaczyć całkkowitą wartość $F = B = \sqrt{X^2 + y^2 + z^2}$
lub skłądową poziomą $H = \sqrt{X^2 + Y^2}$

Skłądowa Z jest rozłożona mniej więcej równoleżnikowo (im bliżej biegunów tym silniejsze pole M)
Definiuje się także 2 kąty:
...
Generalnie jądro ziemii zaqchowuje się jak dynamo kręcąć się wytwarza pole magnetyczne.
Kręcenie się jest najprawdopodobniej spowodowane konwekcją spowodowaną ochładzaniem się planety.
...
\rho = \frac{m}{V} = \frac{p}{R_dT} \\

\frac{dp}{p} = - \frac{g}{R_d} \frac{dz}{T} \\
$$
...
ponieważ paczki powietrza (różniczka objętości) zmieniają... objętość z zatem i temperature poodczas opadania/wznoszenia się,
należy użyć tutaj gradientu temperatury potencjalnej.
...
pV = \frac{m}{M} RT \\
pV = m R_d T \\
\rho = \frac{m}{V} = \frac{p}{R_d T}
$$

co podstawiamy

$$
\frac{\partial p}{\partial z} = - \frac{g}{R_d T} \\
$$

Nasßpnie korzystamy ze wzoru $cp \frac{dT}{T} = R_d \frac{dp}{p}$

$$
\frac{c_p}{R_d * T}\frac{\partial T}{\partial z} =  - \frac{g}{R_d T} \\
