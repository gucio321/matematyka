...
Warunki dirichleta:
1. jest przedziałami monotoniczna na przedziale
2. jest ciągła z wyjątkiem skończonej liczby punktów. w tych punktach nieciągłości punkt ma być średnią arytmetyczną z $\epsilon_-$ i $\epsilon_+$.
3. $f(a) = f(b) = \frac{1}{2}(f(a) + f(b))$
...
c_n = \frac{1}{2l} \int_{-l}^l dx e^{-ik_n x} f(x)
$$
...
Rozwiązywanie:
1. bierzemy jednorodne i wyznaczamy rozwiązanie
2. liczymy rozwiązanie szczegulne dla niejednorodnego (np. pomijamy wtedy stałe całkowania ?)
3. sumujemy 1 i 2
...
$$
\ket y &= \hat L^{-1} \ket f \\
\braket{x|y} &= \bra{x} \hat L^{-1} \ket f \\
\int dx \ket x \bra x &= \mathbb{1} \\
\braket{x|y} &= \int dx' \bra{x} \hat L ^ {-1} \ket{x'} \bra{x'} \ket f \\
y(x) &= \int dx' G(x, x') f(x')
$$ (wyzej)
...
\int dx' f(x') \hat L G(x,x') = - \int dx' f(x') \delta (x - x') \\
\hat L \int dx' f(x') G(x,x') = - \int dx' f(x') \delta (x - x') \\
\hat L y(x) = f(x)
$$

Ostatnie przejście jest z tego co było wyżej. (dobra zrobie wam nawet odnośnik: {eq}`wyzej` )

Możńa podstawić jawnie $\hat L$, całkujemy w granicy $|_{x-\epsilon}^{x+\epsilon}$.

$$
r(x') \lim_{\epsilon \to 0} G(x,x') |_a^b = -1 \\

\lim_{\epsilon \to 0} G(x,x') |_a^b = -\frac{1}{r(x')} \\
$$
